...
| Action | File | Purpose |
|---|---|---|
| Create | `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/DecayFunction.java` | New `@FunctionalInterface` SPI |
| Create | `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/ExponentialDecayFunction.java` | Default CDI implementation with valence multiplier |
| Modify | `runtime/src/main/java/io/quarkiverse/ledger/runtime/config/LedgerConfig.java` | Add `decay()` config group with `flaggedPersistenceMultiplier` |
| Modify | `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/TrustScoreComputer.java` | Add `DecayFunction` constructor; delegate decay calculation |
| Modify | `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/TrustScoreJob.java` | Inject `DecayFunction`; pass to `TrustScoreComputer` |
| Modify | `runtime/src/test/java/io/quarkiverse/ledger/service/TrustScoreComputerTest.java` | Add valence multiplier tests |
...
**Files:**
- Create: `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/DecayFunction.java`
- Create: `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/ExponentialDecayFunction.java`
...
Open `runtime/src/main/java/io/quarkiverse/ledger/runtime/config/LedgerConfig.java`.
...
```bash
git add \
  runtime/src/main/java/io/quarkiverse/ledger/runtime/service/DecayFunction.java \
  runtime/src/main/java/io/quarkiverse/ledger/runtime/service/ExponentialDecayFunction.java \
...
```bash
git add \
  runtime/src/main/java/io/quarkiverse/ledger/runtime/service/TrustScoreComputer.java \
...
| Action | File | Purpose |
|---|---|---|
| Create | `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/TrustGateService.java` | CDI bean for trust threshold queries |
| Create | `runtime/src/test/java/io/quarkiverse/ledger/service/TrustGateServiceTest.java` | Pure unit tests with stub repository |
...
**Files:**
- Create: `runtime/src/test/java/io/quarkiverse/ledger/service/TrustGateServiceTest.java`
...
**Files:**
- Create: `runtime/src/main/java/io/quarkiverse/ledger/runtime/service/TrustGateService.java`
...
```bash
git add \
  runtime/src/main/java/io/quarkiverse/ledger/runtime/service/TrustGateService.java \
  runtime/src/test/java/io/quarkiverse/ledger/service/TrustGateServiceTest.java
...
| Action | File |
|---|---|
| Modify | `~/claude/quarkus-work/quarkus-work-ledger/src/main/java/io/quarkiverse/work/ledger/service/LedgerEventCapture.java` |
| Modify | `~/claude/quarkus-qhorus/runtime/src/main/java/io/quarkiverse/qhorus/runtime/ledger/LedgerWriteService.java` |
| Modify | `~/claude/casehub-engine/casehub-ledger/src/main/java/io/casehub/ledger/service/CaseLedgerEventCapture.java` |

**Prerequisite:** `quarkus-ledger` SNAPSHOT installed — done in Task 7 Step 4.

---

### Task 8: Update quarkus-work — LedgerEventCapture

**File:** `~/claude/quarkus-work/quarkus-work-ledger/src/main/java/io/quarkiverse/work/ledger/service/LedgerEventCapture.java`

- [ ] **Step 1: Find the deriveActorType method**

```bash
grep -n "deriveActorType\|actorType" ~/claude/quarkus-work/quarkus-work-ledger/src/main/java/io/quarkiverse/work/ledger/service/LedgerEventCapture.java
```
...
- [ ] **Step 3: Build and test quarkus-work**

```bash
JAVA_HOME=$(/usr/libexec/java_home -v 26) mvn test -pl quarkus-work-ledger -f ~/claude/quarkus-work/pom.xml 2>&1 | grep -E "Tests run|BUILD|FAIL" | tail -5
```

Expected: tests pass.

- [ ] **Step 4: Commit quarkus-work**

```bash
git -C ~/claude/quarkus-work add quarkus-work-ledger/src/main/java/io/quarkiverse/work/ledger/service/LedgerEventCapture.java
git -C ~/claude/quarkus-work commit -m "feat: use ActorTypeResolver.resolve() in LedgerEventCapture

Replaces local deriveActorType() with the canonical resolver from quarkus-ledger.
...
### Task 9: Update quarkus-qhorus — LedgerWriteService

**File:** `~/claude/quarkus-qhorus/runtime/src/main/java/io/quarkiverse/qhorus/runtime/ledger/LedgerWriteService.java`

- [ ] **Step 1: Find the unconditional ActorType.AGENT assignment**

```bash
grep -n "actorType\|ActorType.AGENT\|sender" ~/claude/quarkus-qhorus/runtime/src/main/java/io/quarkiverse/qhorus/runtime/ledger/LedgerWriteService.java | head -15
```
...
- [ ] **Step 3: Build and test quarkus-qhorus**

```bash
JAVA_HOME=$(/usr/libexec/java_home -v 26) mvn test -pl runtime -f ~/claude/quarkus-qhorus/pom.xml 2>&1 | grep -E "Tests run|BUILD|FAIL" | tail -5
```
...
- [ ] **Step 4: Commit quarkus-qhorus**

```bash
git -C ~/claude/quarkus-qhorus add runtime/src/main/java/io/quarkiverse/qhorus/runtime/ledger/LedgerWriteService.java
git -C ~/claude/quarkus-qhorus commit -m "feat: use ActorTypeResolver.resolve() in LedgerWriteService
...
- [ ] **Step 3: Check if casehub-ledger pom has quarkus-ledger as a dependency**

```bash
grep -n "quarkus-ledger\|quarkiverse.ledger" ~/claude/casehub-engine/casehub-ledger/pom.xml | head -5
```
...
Replaces local deriveActorType() with canonical resolver from quarkus-ledger.
...
grep -rn "deriveActorType\|actorType.*=.*contains\|actorType.*startsWith" \
  ~/claude/quarkus-work ~/claude/quarkus-qhorus ~/claude/casehub-engine ~/claude/claudony \
...
The quarkus-ledger commit that completes this work is the install step (Task 7 Step 4). Add a closing comment:
```bash
gh issue close 53 -R casehubio/quarkus-ledger --comment "Implemented across all consumer repos: quarkus-work (LedgerEventCapture), quarkus-qhorus (LedgerWriteService), casehub-engine (CaseLedgerEventCapture). actorRole audit: consistent across repos, no ActorRoleResolver needed."
```
